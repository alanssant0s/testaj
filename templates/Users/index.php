...
<div class="page-title-box d-sm-flex align-items-center justify-content-between">
    <h4 class="mb-sm-0">Listagem de Usuários</h4>
...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <div id="categories">
                <a href="<?= $this->Url->build("/users/add") ?>" class="btn btn-success"><i class="ri-add-line align-bottom me-1"></i> Adicionar</a>
                <div class="table-responsive">
                    <table class="table align-middle aerojuris-data-table mb-0">
                        <thead class="table-light">
                            <tr>
                                <th class="sort" data-sort="name">Nome</th>
                                <th class="sort" data-sort="plan">Plano</th>
                                <th class="sort" data-sort="auth">Autorização</th>
                        ...
                        <tbody class="list">
                            <?php foreach ($users as $user) : ?>
                                <tr>
                                    ...
                                    <td><?= h($user->auth->name) ?></td>
                                    <td class="actions noExport">
                                        <div class="d-flex gap-2">
                                            <?= $this->Html->link(__('Editar'), ['action' => 'edit', $user->id], ['class' => 'btn btn-sm btn-primary btn-flat']) ?>
                                            <?= $this->Form->postLink(__('Remover'), ['action' => 'delete', $user->id], ['class' => 'btn btn-sm btn-danger btn-flat', 'confirm' => __('Tem certeza que deseja remover o Usuário "{0}"?', $user->name)]) ?>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
