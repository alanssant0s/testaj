...
<div class="page-title-box d-sm-flex align-items-center justify-content-between">
    <h4 class="mb-sm-0">Listagem de Comarcas</h4>

    <div class="page-title-right">
        <ol class="breadcrumb m-0">
            <li class="breadcrumb-item active">Comarcas</li>
        </ol>
    </div>
...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <div id="categories">
                <a href="<?= $this->Url->build("/districts/add") ?>" class="btn btn-success"><i class="ri-add-line align-bottom me-1"></i> Adicionar</a>
                <div class="table-responsive">
                    <table class="table align-middle aerojuris-data-table mb-0">
                        <thead class="table-light">
                            <tr>
                                <th class="sort" data-sort="name">Nome</th>
                                <th class="sort" data-sort="state">Estado</th>
                                <th class="sort" data-sort="city">Cidade</th>
                        ...
                        <tbody class="list">
                            <?php foreach ($districts as $district) : ?>
                                <tr>
                                    ...
                                    <td><?= h($district->city->state->name) ?></td>
                                    <td><?= h($district->city->name) ?></td>
                                    <td class="actions noExport">
                                        <div class="d-flex gap-2">
                                            <?= $this->Html->link(__('Editar'), ['action' => 'edit', $district->id], ['class' => 'btn btn-sm btn-primary btn-flat']) ?>
                                            <?= $this->Form->postLink(__('Remover'), ['action' => 'delete', $district->id], ['class' => 'btn btn-sm btn-danger btn-flat', 'confirm' => __('Tem certeza que deseja remover a Comarca "{0}"?', $district->name)]) ?>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
